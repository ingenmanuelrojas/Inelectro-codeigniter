...
function save_permisos() {
    var url_modulo = "";
    if (op == "guardar") {
        url_modulo = base_url_admin+'configuracion/permisos/store/';
    }else{
        url_modulo = base_url_admin+'configuracion/permisos/update/';
    }
...
function buscar_permiso(id) {
	...
	var url_modulo = base_url_admin+'configuracion/permisos/edit/';
	datos = new FormData($("#frm_permisos")[0]);
    $.post(url_modulo+id,  function(data) {
...
function listar_permisos() {
    url_modulo = base_url_admin+'configuracion/permisos/listado_permisos/';
    $.ajax({
        url     : url_modulo,
        type    : "post",
        dataType : "json",
...
function delete_permiso(id) {
    var url_modulo = base_url_admin+'configuracion/permisos/delete/'+id;
    ...
    alertify.confirm('Mensaje de Confirmación', '¿Estas seguro que deseas eliminar el registro?', function(){
        $.post(url_modulo, {id: id}, function(data) {
            var datos = jQuery.parseJSON(JSON.stringify(data));
