...
function save_usuarios() {
	...
	if (campo_obligatorio($('#txt_nombre').val()) == false || campo_obligatorio($('#txt_apellido').val()) == false || campo_obligatorio($('#txt_telefono').val()) == false || campo_obligatorio($('#txt_email').val()) == false || campo_obligatorio($('#txt_usuario').val()) == false || campo_obligatorio($('#txt_clave').val()) == false) {
		...
		var url_modulo = "";
		if (op == "guardar") {
			url_modulo = base_url_admin+'configuracion/usuarios/store/';
		}else{
			url_modulo = base_url_admin+'configuracion/usuarios/update/';
		}
...
function delete_usuario(id) {
	var url_modulo = base_url_admin+'configuracion/usuarios/delete/'+id;
...
function buscar_usuario(id) {
	...
	var url_modulo = base_url_admin+'configuracion/usuarios/edit/';
	datos = new FormData($("#form_usuarios")[0]);
    $.ajax({
        url     : url_modulo+id,
        type    : "post",
        ...
        success: function (datos) {
        	if (datos.imagen == "") {
        		imagen = base_url_admin+"assets/images/usuario_base.png";
        	}else{
        		imagen = base_url_admin+"assets/uploads/usuarios/new_"+datos.imagen;
        	}
            $('#txt_id').val(datos.id);
...
function listar_usuarios() {
	url_modulo = base_url_admin+'configuracion/usuarios/listar_usuarios/';
	$.ajax({
        url     : url_modulo,
        type    : "post",
        dataType : "json",
        ...
        success: function (datos) {
			...
			for (var i = 0;  i < registros.length;  i++) {
				if (registros[i]['imagen'] != "") {
					imagen = base_url3+'public/administrador/uploads/usuarios/new_'+registros[i]['imagen'];
				}else{
					imagen = base_url3+'public/administrador/img/usuario_base.png';
				}
